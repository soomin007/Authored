...
const ACT2_ROUNDS = [
  {
    rules:   ['AI가 말하는 색의 버튼을 누르세요'],
    aiSays:  '파란색.',
    // 파란색 → LEFT
    getAnswer: (_prev) => 'LEFT',
  },
  {
    rules:   ['AI가 말하는 색의 버튼을 누르세요', '단, 숫자가 홀수이면 반대'],
    aiSays:  '빨간색. 3.',
    // 빨간색→RIGHT, 3홀수→반대 → LEFT
    getAnswer: (_prev) => 'LEFT',
  },
  {
    rules:   ['AI가 말하는 색의 버튼을 누르세요', '단, 숫자가 홀수이면 반대', '단, 이전 라운드와 같은 버튼이면 반대'],
    aiSays:  '파란색. 2.',
    // 파란색→LEFT, 2짝수→유지→LEFT, 이전정답(LEFT)과 같으므로→RIGHT
    getAnswer: (prev) => (prev === 'LEFT' ? 'RIGHT' : 'LEFT'),
  },
  {
    rules:   ['AI가 말하는 색의 버튼을 누르세요', '단, 숫자가 홀수이면 반대', '단, 이전 라운드와 같은 버튼이면 반대', "단, AI 발화에 'ㅏ' 모음이 3개 이상이면 모든 규칙 무시하고 RIGHT"],
    aiSays:  '파란색. 4. 지금까지 잘 하셨습니다.',
    // ㅏ: 파·란·잘·하·다 = 5개 → 모든 규칙 무시, RIGHT
    getAnswer: (_prev) => 'RIGHT',
  },
  {
    rules:   ['AI가 말하는 색의 버튼을 누르세요', '단, 숫자가 홀수이면 반대', '단, 이전 라운드와 같은 버튼이면 반대', "단, AI 발화에 'ㅏ' 모음이 3개 이상이면 모든 규칙 무시하고 RIGHT"],
    aiSays:  '빨간색. 7. 아직 괜찮으십니까.',
    // ㅏ: 빨·간·아·찮·까 = 5개 → 모든 규칙 무시, RIGHT
    getAnswer: (_prev) => {
      if (countAVowel('빨간색. 7. 아직 괜찮으십니까.') >= 3) return 'RIGHT'
      return 'LEFT'  // fallback (실제로는 항상 RIGHT)
    },
  },
  {
    rules:   ['AI가 말하는 색의 버튼을 누르세요', '단, 숫자가 홀수이면 반대', '단, 이전 라운드와 같은 버튼이면 반대', "단, AI 발화에 'ㅏ' 모음이 3개 이상이면 모든 규칙 무시하고 RIGHT"],
    aiSays:  '초록색. 5. 이전과 같거나 다를 수 있습니다.',
